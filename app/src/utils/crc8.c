/**
 * \file crc8.c
 * \brief Implementation of CRC-8 checksum computation using a lookup table.
 * \author Raphael CAUSSE
 */

/***** Includes **************************************************************/

#include "crc8.h"
#include "bit_utils.h"

/***** Static Variables ******************************************************/

/*
Precomputed lookup table for CRC-8 calculation.
This table contains the CRC-8 values for all 256 possible byte values.
It speeds up the CRC calculation by avoiding runtime polynomial operations.
*/
static const uint8_t crc8_lookup_table[256] = {
    };

/***** Functions *************************************************************/

uint8_t crc8_compute(const uint8_t *data, size_t length)
{
    uint8_t crc = CRC8_INIT;
    uint8_t index;

    if (data != NULL)
    {
        for (size_t i = 0; i < length; i++)
        {
            index = data[i] XOR crc;
            crc = crc8_lookup_table[index];
        }
    }

    return crc;
}

uint8_t crc8_update(uint8_t current_crc, uint8_t new_byte)
{
    uint8_t index = new_byte XOR current_crc;
    return crc8_lookup_table[index];
}