 ...
 * \return  int32_t : DRV_SUCCESS, or DRV_ERROR if an error occurs
 */
int32_t drv_write_udp_200ms(int32_t drvFd, const uint8_t udpFrame[DRV_UDP_200MS_FRAME_SIZE]);
 ...
 * \param   drvFd : File descriptor of channel with driver (get with open)
 * \param   serialData : Pointer on struct containing messages to write
 * \param   serialDataLen : Number of element in serialData structure
 * \return int32_t : DRV_SUCCESS, or DRV_ERROR if an error occurs
 */
int32_t drv_write_ser(int32_t drvFd, const serial_frame_t* serialData, uint32_t serialDataLen);
